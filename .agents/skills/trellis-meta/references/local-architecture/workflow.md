...
| Change the wrap-up flow | Update Phase 3 and `finish-work` related descriptions. Current split in this project: Phase 3.4 = conditional knowledge sync, Phase 3.5 = AI-driven code commits (batched, user-confirmed), Phase 3.6 = wrap-up reminder, then `/finish-work` archives and records the session. `/finish-work` refuses to run if the working tree is dirty. |
