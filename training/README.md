...
- Top-level `runs/` is also ignored. It is created by direct Ultralytics
  training/validation/prediction commands when the project root is the working
  directory. Treat it as local experiment output: keep only useful validation
  images or metrics for manual review, and delete empty or obsolete runs.
...
The earlier `models/edgeeye-insulator-v1-opt30-yolov8s-adamw/` local package was
cleaned after its metrics and hashes were recorded in
`dataset/docs/edgeeye-insulator-v1-optimization-report.md`.

`training/runs/` is the expected output location for the project training
scripts. Top-level `runs/` usually means a direct Ultralytics command was run
from the repository root; it can be cleaned when the images or metrics are no
longer needed.
