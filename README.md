...
The realtime page refreshes the existing latest-result API path every second, so no separate camera process is needed when the backend bridge is enabled.
